...
#ifndef OUTPOST_MD5_H
#define OUTPOST_MD5_H
...
{
public:
    typedef unsigned char Digest[MD5_DIGEST_SIZE];
...
private:
    ...
    void context_init();
    void context_hash(const unsigned char[MD5_BLOCK_SIZE]);
...
public:
    /* get MD5 hash, result is in provided Digest */
    void digest(const void *msg, unsigned int len, Digest d);

    /* get MD5 crypted password, result is malloced, so you must free() it */
