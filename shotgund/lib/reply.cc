    ...
    allrrs(NULL), allrrsCur(0)
{
    ...
    parse_RR_funcs[17] = &AsyncDNSReply::parse_NAPTR_RR;
    parse_RR_funcs[18] = &AsyncDNSReply::parse_SRV_RR;
...
int AsyncDNSReply::parseinternal()
{
    ...
    if(anc){
        ...
        for(i = 0; i<anc; i++){
            ...
            if((type>16 || type == 0) && type != RFC1035_TYPE_AAAA && type != RFC1035_TYPE_NAPTR && type != RFC1035_TYPE_SRV){
                return -1;
            }
            ...
            if(type == RFC1035_TYPE_AAAA)
                ttype = 17;
            if(type == RFC1035_TYPE_NAPTR)
                ttype = 18;
            if(type == RFC1035_TYPE_SRV)
                ttype = 19;
...
RFC1035_RR * AsyncDNSReply::parse_NAPTR_RR(u_int16_t rdlength, u_int16_t from)
{
    /*
    if(from >= (unparsedBufLen-15)){
        return NULL;
    ...
    */
    ...
    RFC1035_NAPTR_RR * naptr_rr = (RFC1035_NAPTR_RR*)mempool->alloc(sizeof(RFC1035_NAPTR_RR));

    //memcpy(&naptr_rr->address, &unparsedBuf[from], sizeof(naptr_rr->address));

    naptr_rr->prevRR = prevRR[17]; prevRR[17] = naptr_rr;

    return naptr_rr;
}

RFC1035_RR * AsyncDNSReply::parse_SRV_RR(u_int16_t rdlength, u_int16_t from)
{
    /*
    if(from >= (unparsedBufLen-15)){
        return NULL;
    ...
    */
    ...
    RFC1035_SRV_RR * srv_rr = (RFC1035_SRV_RR*)mempool->alloc(sizeof(RFC1035_SRV_RR));

    //memcpy(&naptr_rr->address, &unparsedBuf[from], sizeof(naptr_rr->address));

    srv_rr->prevRR = prevRR[18]; prevRR[18] = srv_rr;
